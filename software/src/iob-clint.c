...
void clint_set_timer(unsigned long long time) {
  (*(volatile uint32_t *)(clint_base + MTIME_BASE)) =
      (uint32_t)(time & 0x0FFFFFFFFUL);
  (*(volatile uint32_t *)(clint_base + MTIME_BASE + 4)) =
      (uint32_t)(time >> 32);
}
...
uint64_t clint_get_timer() {
  ...
  read_time |= (uint64_t)(*(volatile uint32_t *)(clint_base + MTIME_BASE + 4))
               << 32;
...
void clint_set_timercmp(unsigned long long time, int hart) {
  (*(volatile uint32_t *)(clint_base + MTIMECMP_BASE + 8 * hart + 4)) =
      0xFFFFFFFF;
  (*(volatile uint32_t *)(clint_base + MTIMECMP_BASE + 8 * hart)) =
      (uint32_t)(time & 0x0FFFFFFFFUL);
  (*(volatile uint32_t *)(clint_base + MTIMECMP_BASE + 8 * hart + 4)) =
      (uint32_t)(time >> 32);
}
...
uint64_t clint_get_timercmp(int hart) {
  ...
  read_time =
      (uint64_t)(*(volatile uint32_t *)(clint_base + MTIMECMP_BASE + 8 * hart));
  read_time |= (uint64_t)(*(volatile uint32_t *)(clint_base + MTIMECMP_BASE +
                                                 8 * hart + 4))
               << 32;
...
void mtimer_set_raw_time_cmp(uint64_t clock_offset) {
  ...
  volatile uint64_t *mtimecmp =
      (volatile uint64_t *)(clint_base + MTIMECMP_BASE);
  *mtimecmp = new_mtimecmp;
#else
  volatile uint32_t *mtimecmpl =
      (volatile uint32_t *)(clint_base + MTIMECMP_BASE);
  volatile uint32_t *mtimecmph =
      (volatile uint32_t *)(clint_base + MTIMECMP_BASE + 4);
